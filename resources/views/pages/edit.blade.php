...
@section('content')
  <div class="content">
        ...
        @endif
        <form class="create-form" action="{{ action('PostsController@update', $post->unique_id)}}" method="POST" enctype="multipart/form-data">

          {{csrf_field()}}
          <div class="form-group">
              <label for="title">Product Name</label>
              <input type="text" class="form-control" id="product-name" aria-describedby="name" placeholder="Product Name" name="name" value="{{$post->name}}">
            </div>
            <div class="form-group">
                <label for="description">Product Description</label>
                <textarea cols="10" rows="10" class="form-control"  placeholder="Description" name='description'>{{$post->description}}</textarea>
              </div>
              <div class="row">
                <div class="form-group form-mini">
                    <label for="stock">Stock</label>
                    <input type="text" name="stock" class="form-control" value="{{$post->stock}}">
